...
Program:
while not STOP0:
    NAV0=NAVIGATE(image=IMAGE, goal='kitchen')
...
Program:
while not STOP0:
    NAV0=NAVIGATE(image=IMAGE,goal='[10,0,-30]')
    LOC0=LOCATE(pos=NAV0)
    EVAL0=EVAL(expr="'STOP' if {LOC0} else 'NAVIGATE'")
    STOP0=STOP(var=EVAL0)
""",
...
while not STOP0:
    ...
    STOP0=STOP(var=EVAL0)
while not STOP1:
    NAV1=NAVIGATE(image=IMAGE,goal='[0,0,2]')
...
Program:
while not STOP0:
    NAV0=NAVIGATE(image=IMAGE,goal='bathroom')
    LOC0=LOCATE(pos=NAV0)
    EVAL0=EVAL(expr="'STOP' if {LOC0} else 'NAVIGATE'")
    STOP0=STOP(var=EVAL0)
while not STOP1:
    NAV1=NAVIGATE(image=IMAGE,goal='bedroom')
    LOC1=LOCATE(pos=NAV1)
    EVAL1=EVAL(expr="'STOP' if {LOC1} else 'NAVIGATE'")
    STOP1=STOP(var=EVAL1)
""",
